...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 0, 0.0005);
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 4, 5e-05);
...
static void XILoutputTID01(SimStruct *S, int tid)
{
...

  {
    ...
    if (IOBufferPtr != NULL) {
      void * pXILService = (void *) ssGetPWorkValue(S, 9);
      if (pXILService != NULL) {
        if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, (14) * 2, 0)!=
            XILHOSTAPPSVC_SUCCESS) {
          return;
        }                              /* if */

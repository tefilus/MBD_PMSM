 ...
 * Component Simulink Path: Controller1
 * Simulation Mode: SIL
 *
 */

#define S_FUNCTION_NAME                Controller1_sbs
...
static boolean_T processResponseError(SimStruct * S)
{
  ...
  void * pMemUnitTransformer = (void *) ssGetPWorkValue(S, 11);
  void * pXILService = (void *) ssGetPWorkValue(S, 9);
...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
...

    {
      ...
      rtIOStreamDataPtr->targetSendBufferSizeBytes = 50000 + 2;
...

  {
    ...
    {
      mxArray * codeGenComponent = mxCreateString("Simulation_test");
      mxArray *rhs[3];
      const char * simulinkBlockPath = ssGetPath(S);
      rhs[ 0 ] = mxCreateString(
        "@coder.connectivity.SimulinkInterface.getSILPILInterface");
...

                      {
...

                        {
                          if (xilReadData(S, (uint8_T*)simDataMemUnitPtr,
                                          num_elements, MEM_UNIT_SINGLE_TYPE) !=
                              XILHOSTAPPSVC_SUCCESS) {
                            return;
                          }            /* if */
                        }
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  /* using port based sample times */
  ssSetInputPortSampleTime(S, 1, 2.5e-05);
  ...
  /* using port based sample times */
  ssSetInputPortSampleTime(S, 2, 2.5e-05);
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 0, 5e-07);
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 2, 5e-05);
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 3, 0.0005);
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 4, 0.0005);
...
static void mdlSetupRuntimeResources(SimStruct *S)
{
...

  {
    ...
    ssSetPWorkValue(S, 11, pMemUnitTransformer);
  }
...
static void mdlStart(SimStruct *S)
{
  ...
  /* no solver check required for multitasking scheduling */
...
{
  {
    ...
    if (IOBufferPtr != NULL) {
      void * pXILService = (void *) ssGetPWorkValue(S, 9);
      if (pXILService != NULL) {
        if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, (10) * 2, 0)!=
            XILHOSTAPPSVC_SUCCESS) {
          return;
        }                              /* if */
...
static void XILoutputTID03(SimStruct *S, int tid)
{
  {
    XIL_IOBuffer_T * IOBufferPtr = (XIL_IOBuffer_T *) ssGetPWorkValue(S, 1);
    if (IOBufferPtr != NULL) {
      ...
      if (pXILService != NULL) {
        ...
        if (IOBufferPtr->data != NULL) {
...

          {
            {
              uint8_T * simDataMemUnitPtr;
              uint32_T commandDataFcnidTID[2] = { 0, 3 };
...
static void XILoutputTID04(SimStruct *S, int tid)
{
  {
    XIL_IOBuffer_T * IOBufferPtr = (XIL_IOBuffer_T *) ssGetPWorkValue(S, 1);
    if (IOBufferPtr != NULL) {
      ...
      if (pXILService != NULL) {
        ...
        if (IOBufferPtr->data != NULL) {
...

          {
            {
              uint8_T * simDataMemUnitPtr;
              uint32_T commandDataFcnidTID[2] = { 0, 4 };
...
static void XILoutputTID05(SimStruct *S, int tid)
{
  {
    XIL_IOBuffer_T * IOBufferPtr = (XIL_IOBuffer_T *) ssGetPWorkValue(S, 1);
    if (IOBufferPtr != NULL) {
      void * pXILService = (void *) ssGetPWorkValue(S, 9);
      if (pXILService != NULL) {
        if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, (7) * 2, 0)!=
            XILHOSTAPPSVC_SUCCESS) {
          return;
        }                              /* if */
        ...
        if (IOBufferPtr->data != NULL) {
...

          {
            {
              uint8_T * simDataMemUnitPtr;
              uint32_T commandDataFcnidTID[2] = { 0, 5 };
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
  ...
  /* Multirate, multitasking scheduling */
  /* check for sample time hit associated with task 3 */
  if (ssIsSampleHit(S, 2, tid)) {
    XILoutputTID03(S, tid);
  }                                    /* if */

  /* Multirate, multitasking scheduling */
  /* check for sample time hit associated with task 4 */
  if (ssIsSampleHit(S, 3, tid)) {
    XILoutputTID04(S, tid);
  }                                    /* if */

  /* Multirate, multitasking scheduling */
  /* check for sample time hit associated with task 5 */
  if (ssIsSampleHit(S, 4, tid)) {
    XILoutputTID05(S, tid);
  }                                    /* if */
}
